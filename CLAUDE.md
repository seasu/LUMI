...
# 啟動 Firebase 本地模擬器（Functions + Firestore + Auth）
...
> **注意**：`flutter run` 用於本機開發迭代是允許的。
> 正式版建置（IPA / AAB）嚴格禁止在本機執行，必須透過 GitHub Actions。
...
**本機開發**：在 `lib/` 目錄建立 `.env`（已加入 `.gitignore`），格式如下：
...
3. **安全第一**：發現任何違反上方安全規範的程式碼，立即標記並修正，不等使用者提出。
4. **ADR 優先**：若新需求與 ADR 衝突，先提出討論，不自行推翻已確認的架構決策。
5. **Commit 格式**：所有 commit 必須符合 Conventional Commits 規範（見上方）。
