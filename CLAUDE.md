...
> **Flutter Web**：本機除錯需自行帶上與 Firebase／Google 相關的 `--dart-define`（對照 `lib/core/config/firebase_options.dart`、`.env.example`）；GitHub Pages 正式站由 Actions secrets 注入。瀏覽器 **DevTools → Console** 會輸出 **`webConsoleInfo`** JSON（`tag`: `bootstrap`／`sync`／`thumbnail`），便於對照同步與縮圖刷新問題（實作見 `lib/core/logging/web_console_log*.dart`）。
...
每次改動程式碼（前端或後端）時，**必須同步更新版本資訊與 PRD 版本歷史**，不得只改程式不更新文件。
...
- 在 `LUMI_PRD.md` 中維護「版本歷史」章節（建議用表格），且**固定放在 PRD 最後一節**。
- 每次更新至少記錄：
  - 版本號（前端 / 後端）
  - 日期
  - 本次改動摘要（1–3 點）
  - 影響範圍（例如：Auth、Wardrobe Sync、UI、Functions）
